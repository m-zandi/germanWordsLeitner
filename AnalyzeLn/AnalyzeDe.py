...
from mainV2.KeysNMsgs import KeysNMsgsDe as Sending
...
class KindOfUser:
    ...
    def identifyUserAndMsg(self):  
        ...
        # if self.id == 73543260 or self.id == 927779469:
        # if self.id == 73543260 or self.id == 130405462:
                    ...
                    try:    
                            ...
                            if self.obj_DbC.Msg().isTheLastOneRepeatedMsg(self.id,"User") is True:
                                ...
                                if userIdentify is False:                  
                                    ...
                                    repeat = 1  
                            if self.obj_DbC.Msg(self.id,self.msg).noneKeyboardMsgsDe(self.msg,"User") and repeat == 0  :
                                ...
                                #done پیام استفاده از کیبورد
                                    ...
                                    if self.msg == Btn().startLearning :
                                        if userIdentify is False:  
                                                    ...
                                                    self.obj_DbC.UILanguage(self.id).saveUIL(BtnS().keyNMsgDe)
                                    ...
                                    #             self.obj_DbC.Counter(self.id).putZeroToCounter()
                                    #             withoutEmoj = ""
                                    #             dictMsg ={BtnS().persianNew:"persian", BtnS().deutschNew:"deutsch",BtnS().englishNew:"english",BtnS().synonymNew:"synonym"}
                                    ...
                                    #             # self.obj_DbC.RaveshYadgiri(self.id).saveWayBool(withoutEmoj)
                                    #             #planA
                                    #             self.obj_DbC.RaveshYadgiri(self.id,withoutEmoj).saveWay()
                                    #             #make package words 
                                    #             self.obj_DbC.Vazhegan(self.id,self.msg).mixWords()
                                    #             # self.obj_Vazhegan.mixWords()
                                    ...
                                    #             # wordsArray = self.obj_DbC.Vazhegan(self.id).getTodayWords()
                                    #             # print(f"wordsArray = {wordsArray}")
                                    #             # numAll = self.obj_DbC.Vazhegan(self.id).getWordsLength(wordsArray)
                                    #             # #get word
                                    ...
                                    #             # way = self.obj_DbC.RaveshYadgiri(self.id).wayWithoutComma(way)
                                    #             reportNum = self.obj_DbC.Report(self.id).getReportNum()
                                    #             #standardized word
                                    #             standardizedWord = self.obj_DbC.Vazhegan(self.id).standardizeWord(word,reportNum)
                                    #             #نگاشتن تاریخ یادگیری و تمرین نوبت پسین  و بعدی در فردا
                                    ...
                                    #                 # wKind,numW,numAll,word,content,chapter,wordsPage,linkWord
                                ...
                                ###################### در فهرست آغازین و اصلی ###################
                                    ...
                                    if self.msg == Btn().reportWordsPartions:
                                            if userIdentify is True:
                                                ...
                                                msg2 = self.obj_DbC.Report(self.id).reportWordsSectionsPercentageDe()
                                    ...
                                    if self.msg == Btn().account:
                                        if userIdentify is True:
                                            outpuTodayDateNTime,output = self.obj_DbC.Report(self.id).getAccoutDetailsDe()
...
class Admin(AnlaizeAbstract):
    ...
    def msgAnlizer (self):
        ...
        try:    
                ...
                if  self.obj_DbC.Msg(self.id,self.msg).noneKeyboardMsgsDe(self.msg,"Admin") and repeat == 0:
                    ...
                    if userIdentify is False:
                        ...
                        if self.obj_DbC.Admin(self.id).verifySendToAllMsg() is True and self.obj_DbC.Admin(self.id).msgKeyBotComparisonDe(self.msg) is True:
                        ...
                        if self.msg == BtnS().oneNew or self.msg == BtnS().twoNew or self.msg == BtnS().threeNew or self.msg == ABtnS().fiveNew or  self.msg == ABtnS().tenNew or self.msg == ABtnS().fifteenNew or self.msg == ABtnS().twentyNew :
                            if userIdentify is False:
                                    ...
                                    # در صورتی که کاربر پیشتر نام نویسی کرده باشد
                            else:
                                # در صورتی که کاربر پیشتر نام نویسی کرده باشد
                                #########
                                tomorrowNextTraining =self.obj_DbC.DateArrange().getTomorrowNextTraining(self.id)
