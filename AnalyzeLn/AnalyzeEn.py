...
from mainV2.KeysNMsgs import KeysNMsgsEn as Sending
...
class Karbar(AnlaizeAbstract):
        ...
        try:
            ...
            if userIdentify and channelMemVeri is False:
                self.obj_KNM.Start(self.id,self.firstName,self.msg).memberChannel(channel)
                    ...
                    try:    
                            repeat = 0  
                            # self.obj_DbC.Msg(self.id,self.msg).arrangeMsgArray()
                            # self.obj_DbC.Msg(self.id,self.msg).saveMsgArraytoDb()
                            ...
                            if self.obj_DbC.Msg().isTheLastOneRepeatedMsg(self.id,"User") is True:
                                ...
                                if userIdentify is False:                  
                                    ...
                                    repeat = 1  
                            if self.obj_DbC.Msg(self.id,self.msg).noneKeyboardMsgsEn(self.msg,"User") and repeat == 0  :
                                ...
                                #done پیام استفاده از کیبورد
                                    ...
                                    if self.msg == BtnS().keyNMsgPer:      
                                        self.obj_DbC.UILanguage(self.id).saveUIL(self.msg)
                                        if userIdentify is False:
                                                KNMPer.Start(self.id,self.firstName,self.msg).sendKeyAndMessageFNVAUIselected()
                                        else:
                                                KNMPer.AghazYadgiriVazhehayeKetab(self.id,self.firstName,self.msg).sendKeyAndMessageFAVA()
                                    ...
                                    if self.msg == Btn().startLearning :
                                        if userIdentify is False:
                                                    ...
                                                    self.obj_DbC.UILanguage(self.id).saveUIL(BtnS().keyNMsgEn)
                                ...
                                ###################### در فهرست آغازین و اصلی ###################
                                                ...
                                                if self.obj_DbC.DateArrange().getAutomateMsgFlag(self.id) is True:
                                                            ...
                                                            if reportNum is None:
                                                                ...
                                                                if   5<reportNum: 
                                                                    ...
                                                                    self.obj_KNM.Inline(self.id,self.data).dataWord(way,wordCard,Btn().dailyLearnWords)                                                    # self.obj_DbC.Vazhegan(self.id).eraseTodayWords()
                                    ...
                                    if self.msg == Btn().reportWordsPartions:
                                            if userIdentify is True:
                                                ...
                                                msg2 = self.obj_DbC.Report(self.id).reportWordsSectionsPercentageEn()
                                    ...
                                    if self.msg ==  Btn().account:
                                        if userIdentify is True:
                                            outpuTodayDateNTime,output = self.obj_DbC.Report(self.id).getAccoutDetailsEn()
                                    ...
                                    if self.msg ==Btn().deleteBot:
                                        if userIdentify is True:
                                            self.obj_KNM.Hazfbot(self.id,self.firstName,self.msg).sendHRKeyAndMessageYesNo()     
                                        else:
                                            self.obj_KNM.Start(self.id,self.firstName,self.msg).sendKeyAndMessageFNVAUINakhostin()



                                    #در فهرست آغازین و اصلی .....#"حذف ربات 🗑"
                                    if self.msg == Btn().yesDelete :
                                        if userIdentify is True:
                                                ...
                                                self.obj_KNM.Hazfbot(self.id,self.firstName,self.msg).sendYesKeyAndMessageFNVA()
    ...
    def dataAnalizer(self):
        ...
        try:
            ...
            self.obj_DbC.Report(self.id).firstLeitnerBoxStatistics(self.data)
            # self.obj_DbC.Report(self.id).leitnerBoxStatistics(self.data)
                            ...
                            # if userIdentify is True:
                                    # if self.obj_DbC.DateArrange().getRightWrongLampFlag(self.id) is True:
                                            ...
                                            #آزمون آخرین بودن یا نبودن واژه 
                                                ...
                                                #################  report part  
                                                if self.obj_DbC.Counter(self.id).lastWordVerification() == False:    
                                                    #save last box values in temporary
                                                    #TODO geting  card Number,firstName,language,id,first,last user 
...
class Admin(AnlaizeAbstract):
    ...
    def msgAnlizer (self):
        ...
        try:    
                ...
                if  self.obj_DbC.Msg(self.id,self.msg).noneKeyboardMsgsEn(self.msg,"Admin") and repeat == 0:
                    ...
                    if userIdentify is False:
                        ...
                        if self.obj_DbC.Admin(self.id).verifySendToAllMsg() is True and self.obj_DbC.Admin(self.id).msgKeyBotComparisonEn(self.msg) is True:
                        ...
                        if self.msg == BtnS().keyNMsgPer:        
                            self.obj_DbC.UILanguage(self.id).saveUIL(self.msg)
                            if userIdentify is False:
                                    KNMPer.AdminStart(self.id,self.firstName,self.msg).sendKeyAndMessageFNVAUIselected()
                            else:
                                    KNMPer.AdminAghazYadgiriVazhehayeKetab(self.id,self.firstName,self.msg).sendKeyAndMessageFAVA()
                    ...
                    ###################### دکمه های مشترک میان فهرست ها ###################
                        ...
                        #                         #################  report part  
                        #                         if self.obj_DbC.Counter(self.id).lastWordVerification() == False:    
                        #                         # if self.obj_Counter.lastWordVerification() == False:
                        #                             # self.obj_DbC.DateArrange().saveFalseRightWrongLampFlag(self.id)
                        #                             # self.obj_DbC.DateArrange().saveFalseAutomateMsgFlag(self.id)
                        #                             # all = self.obj_DbC.Report(self.id).allWordsNum()
                        #                             # right = self.obj_DbC.Report(self.id).rightWordsNum()
                        #                             # wrong = self.obj_DbC.Report(self.id).wrongWordsNum()
                        #                             # #اضافه کردن یک رقم به عدد و شمار گزارش <reportNum>
                        #                             # self.obj_DbC.Report(self.id).addOneToReportNum()
                        #                             # #نگاشتن تاریخ یادگیری و تمرین نوبت پسین  و بعدی در فردا
                        #                             # self.obj_DbC.DateArrange().saveTomorrowNextTraining(self.id)
                        #                             # tomorrowNextTraining = self.obj_DbC.DateArrange().getTomorrowNextTraining(self.id)
                        #                             # houNMTraining = tomorrowNextTraining.strftime("%H:%M")

                        #                             # wrongWordsNpages = self.obj_DbC.Report(self.id).wrongWordsWithPages()
                        #                             # wordsSectionPosition = self.obj_DbC.Report(self.id).wordsSectionPosition()
                        #                             # self.obj_DbC.Counter(self.id).putZeroToCounter()
                        #                             # # self.obj_Counter.putZeroToCounter()  
                        #                             # #گرفتن سال ، ماه نویسه ای ، روز
                        #                             # # monthAlpha,year,month,day
                        ...
                        #                             # # here
                        #                             # #get word num
                        #                             # numW = self.obj_DbC.Counter(self.id).getCounter() + 1
                        #                             # #get word
                        #                             # todayWords = self.obj_DbC.Vazhegan(self.id).getTodayWords()
                        #                             # # todayWords = self.obj_Vazhegan.getTodayWords()
                        #                             # #####
                        #                             # word = self.obj_DbC.Vazhegan(self.id).getOneRightWord(todayWords)
                        #                             # # word =  self.obj_Vazhegan.getOneRightWord(todayWords)
                        #                             # #get number of all words
                        #                             # numAll = self.obj_DbC.Vazhegan(self.id).getWordsLength(todayWords)
                        #                             # #kind of word
                        #                             # wKind = self.obj_DbC.Vazhegan(self.id).kindOfWord(word)
                        #                             # #get words page
                        #                             # wordsPage = self.obj_DbC.Book(self.id).getOnePage(word)
                        #                             # #get content and chapter abbrevation
                        #                             # # content,chapter = self.obj_DbC.Book(self.id).makeMixAbbContentChap(word)
                        #                             # # cotentNChap = f"{content}-{chapter}"
                        #                             # content = self.obj_DbC.Book(self.id).getContent(word)
                        #                             # chapter = self.obj_DbC.Book(self.id).getChapter(word)
                        #                             # # cotentNChap = f"{content}\n{chapter}"
                        #                             # #planA
                        #                             # way = self.obj_DbC.RaveshYadgiri(self.id).getWay()
                    ...
                    ###################### در فهرست آغازین و اصلی ###################
                                    ...
                                    if self.obj_DbC.DateArrange().getAutomateMsgFlag(self.id) is True:
                                                ...
                                                if reportNum is None:
                                                    ...
                                                    if   5<reportNum:
                                                            ...
                                                            wordCard = MNV(self.firstName).wordCard(wKind,numW,numAll,standardizedWord,content,chapter,wordsPage,wordLink)
                                                            self.obj_KNM.AdminInline(self.id,self.data).dataWord(way,wordCard,Btn().dailyLearnWords)                                                    # self.obj_DbC.Vazhegan(self.id).eraseTodayWords()
                                                    else:
                                                        
                                                            wordCardWithGuide = MNV(self.firstName).wordCardWithGuide(way,wKind,numW,numAll,standardizedWord,content,chapter,wordsPage,wordLink)
                                                            self.obj_KNM.AdminInline(self.id,self.msg).dataWordWithGuide(way,wordCardWithGuide,Btn().dailyLearnWords)
                                                    ...
                                                    if   5<reportNum:
                                                        ...
                                                        # way,wKind,numW,numAll,word,content,chapter,wordsPage,linkWord
                                                        wordCardWithGuide = MNV(self.firstName).wordCardWithGuide(way,wKind,numW,numAll,standardizedWord,content,chapter,wordsPage,wordLink)
                                                        self.obj_KNM.AdminInline(self.id,self.msg).dataWordWithGuide(way,wordCardWithGuide,Btn().dailyLearnWords)
                                    else:
                                            #########
                        ...
                        if self.msg == Btn().chapterNSection:
                                if userIdentify is True:
                                        ...
                                        try:
                                            ...
                                            chNSWCB = [BtnS().deutschChapNSCB,BtnS().englishChapNSCB,BtnS().synonymChapNSCB,BtnS().persianEnChapNSCB]
                                            if lengthIF == 1:
                                                self.obj_KNM.AdminInline(self.id,self.firstName,self.msg).reviewOne(chNSWCB,way,length,msgCHNS,self.msg)
                                            else:
                                                self.obj_KNM.AdminInline(self.id,self.firstName,self.msg).reviewMoreThanOne(counter,chNSWCB,way,length,msgCHNS,self.msg)
                                            counterIF = self.obj_DbC.Counter(self.id).getCounter()
                                            print(f"counterIF = {counterIF}")
                                            # here
                                        except:
                                            self.obj_KNM.AdminMorureVazhehhayeGozashteh(self.id,self.firstName,self.msg).sendWeakWordsKeyAndMessageNoWordOtherReview()
                                        ###############   
                                else:
                                        self.obj_KNM.AdminStart(self.id,self.firstName,self.msg).sendKeyAndMessageFNVAUINakhostin()
                        ...
                        if self.msg ==Btn().deleteBot:
                            if userIdentify is True:
                                    self.obj_KNM.AdminHazfbot(self.id,self.firstName,self.msg).sendHRKeyAndMessageYesNo()     
                            else:
                                self.obj_KNM.AdminStart(self.id,self.firstName,self.msg).sendKeyAndMessageFNVAUINakhostin()
                        ...
                        if self.msg == Btn().yesDelete :
                            if userIdentify is True:
                                    ...
                                    # حذف ربات 🗑
                                    self.obj_DbC.DeleteBot(self.id).deleteBot()
                                    self.obj_KNM.AdminHazfbot(self.id,self.firstName,self.msg).sendYesKeyAndMessageFNVA()
                                        ...
                                        # if self.obj_DbC.DateArrange().getRightWrongLampFlag(self.id) is True:
                                                ...
                                                #آزمون آخرین بودن یا نبودن واژه 
                                                    ...
                                                    if self.obj_DbC.Counter(self.id).lastWordVerification() == False:    
                                                        ...
                                                        #TODO geting  card Number,firstName,language,id,first,last Admin 
                                                        self.obj_DbC.Report(self.id).lastLeitnerBoxStatistics(self.data)
                                                        lan = self.obj_DbC.UILanguage(self.id).getUIL()
                                                        first,last = self.obj_DbC.Report(self.id).getLeitnerBoxStatistics()
                                                        # print(f"{first = } {last = } {lan = } {self.id = } {self.firstName = }")
